...
final class Responsive_Block_Swap {
	...
	private function __construct() {
		...
		add_action("enqueue_block_assets", [$this, 'responsive_block_swap_enqueue_block_assets']);
		add_action("admin_enqueue_scripts", [$this, 'responsive_block_swap_admin_scripts']);
	...
	public function responsive_block_swap_enqueue_block_assets() {
		$assets = include(RESPONSIVE_BLOCK_SWAP_PATH . 'build/index.asset.php');
		wp_enqueue_style('responsive-block-swap-style', RESPONSIVE_BLOCK_SWAP_URL . 'build/index.css', [], $assets['version']);
	}

	public function responsive_block_swap_admin_scripts($screen) {
		if (is_admin()) {
			wp_localize_script("wp-block-editor", "responsiveBlockSwap", [
				"screen" => $screen
			]);
		}
	}
	...
	function responsive_block_swap_render_block( $block_content, $block ) {
		...
		$content = !empty($classes) ? preg_replace('/class="(.*?)/', 'class="'. $classes .' ', $block_content, 1) : $block_content;
		return $content;
	}
